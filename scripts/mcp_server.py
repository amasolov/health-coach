...
class BearerAuthMiddleware(Middleware):
    ...
    async def _authenticate(self, context: MiddlewareContext) -> None:
        ...
        user_id = await anyio.to_thread.run_sync(
            functools.partial(health_tools.resolve_user_id, user["slug"]),
        )
        if user_id is None:
...
async def _wrap(fn, *args, **kwargs):
    """Run a sync health_tools function in a worker thread.

    Prevents blocking the async event loop while psycopg2 queries execute.
    Converts ValueError into ToolError for MCP clients.
    """
    try:
...
@mcp.tool
async def get_fitness_summary(ctx: Context) -> dict:
    ...
    projection of CTL."""
    return await _wrap(health_tools.get_fitness_summary, _uid(ctx))
...
async def get_training_load(
    ...
    return await _wrap(health_tools.get_training_load, _uid(ctx), start_date, end_date, days)
...
async def get_activities(
    ...
    return await _wrap(health_tools.get_activities, _uid(ctx), start_date, end_date, days, sport, limit)


@mcp.tool
async def get_activity_detail(ctx: Context, activity_time: str) -> dict:
    """Get full detail for a single activity by its timestamp (ISO format)."""
    return await _wrap(health_tools.get_activity_detail, _uid(ctx), activity_time)
...
async def get_body_composition(
    ...
    return await _wrap(health_tools.get_body_composition, _uid(ctx), start_date, end_date, days)
...
async def get_vitals(
    ...
    return await _wrap(health_tools.get_vitals, _uid(ctx), start_date, end_date, days)


# ===== ZONES & PROFILE =====

@mcp.tool
async def get_training_zones(ctx: Context) -> dict:
    """Get current training zones: heart rate, running power, cycling power,
    and running pace with absolute lower/upper bounds."""
    return await _wrap(health_tools.get_training_zones, _uslug(ctx))


@mcp.tool
async def get_athlete_profile(ctx: Context) -> dict:
    """Get the athlete's profile: goals, thresholds, body composition,
    training status, and treadmill zone-to-speed mapping."""
    return await _wrap(health_tools.get_athlete_profile, _uslug(ctx))


# ===== STRENGTH =====

@mcp.tool
async def get_strength_sessions(
    ctx: Context,
    start_date: str = "",
    end_date: str = "",
    days: int = 30,
    ...
    return await _wrap(
        health_tools.get_strength_sessions,
...
@mcp.tool
async def list_treadmill_templates() -> list[dict]:
    """List available treadmill workout templates with name, duration, and
    step count."""
    return await _wrap(health_tools.list_treadmill_templates)


@mcp.tool
async def generate_treadmill_workout(ctx: Context, template_key: str) -> dict:
    ...
    return await _wrap(health_tools.generate_treadmill_workout, _uslug(ctx), template_key)


# ===== GARMIN AUTHENTICATION =====

@mcp.tool
async def garmin_auth_status(ctx: Context) -> dict:
    ...
    return await _wrap(health_tools.garmin_auth_status, _uslug(ctx), email)


@mcp.tool
async def garmin_authenticate(ctx: Context) -> dict:
    ...
    return await _wrap(health_tools.garmin_authenticate, _uslug(ctx), email, password)


@mcp.tool
async def garmin_submit_mfa(ctx: Context, mfa_code: str) -> dict:
    """Complete Garmin Connect MFA authentication with the code the user
    received (via email or authenticator app)."""
    return await _wrap(health_tools.garmin_submit_mfa, _uslug(ctx), mfa_code)


# ===== ATHLETE PROFILE SETUP =====

@mcp.tool
async def garmin_fetch_profile(ctx: Context) -> dict:
    ...
    return await _wrap(health_tools.garmin_fetch_profile, _uslug(ctx))


@mcp.tool
async def generate_fitness_assessment(
    ctx: Context,
...
async def update_athlete_profile(
    ...
    return await _wrap(health_tools.update_athlete_profile, _uslug(ctx), field_path, value)


# ===== GOALS & ONBOARDING =====

@mcp.tool
async def get_onboarding_questions(ctx: Context) -> dict:
    ...
    return await _wrap(health_tools.get_onboarding_questions, _uslug(ctx))


@mcp.tool
async def set_user_goals(ctx: Context, goals: dict) -> dict:
    ...
    return await _wrap(health_tools.set_user_goals, _uslug(ctx), goals)


@mcp.tool
async def get_user_goals(ctx: Context) -> dict:
    """Get the user's current goals, preferences, and constraints."""
    return await _wrap(health_tools.get_user_goals, _uslug(ctx))


# ===== ACTION ITEMS =====

@mcp.tool
async def get_action_items(ctx: Context, status_filter: str = "") -> dict:
    ...
    return await _wrap(health_tools.get_action_items, _uslug(ctx), status_filter)
...
async def add_action_item(
    ...
    return await _wrap(health_tools.add_action_item, _uslug(ctx), title, description, category, priority, due)
...
@mcp.tool
async def complete_action_item(ctx: Context, item_id: str, note: str = "") -> dict:
    """Mark an action item as completed. Optionally add a completion note
    (e.g. 'LTHR measured at 168 bpm')."""
    return await _wrap(health_tools.complete_action_item, _uslug(ctx), item_id, note)


# ===== INTEGRATIONS & HARDWARE REGISTRY =====

@mcp.tool
async def get_supported_integrations(ctx: Context, category: str = "") -> dict:
    ...
    return await _wrap(health_tools.get_supported_integrations, category)


@mcp.tool
async def set_user_integrations(ctx: Context, integrations: list[str], equipment_notes: dict | None = None) -> dict:
    ...
    """
    return await _wrap(health_tools.set_user_integrations, _uslug(ctx), integrations, equipment_notes)


@mcp.tool
async def get_user_integrations(ctx: Context) -> dict:
    """Get the user's configured integrations and hardware."""
    return await _wrap(health_tools.get_user_integrations, _uslug(ctx))


# ===== iFit INTEGRATION =====

@mcp.tool
async def recommend_ifit_workout(ctx: Context) -> dict:
    ...
    return await _wrap(health_tools.recommend_ifit_workout, _uslug(ctx))


@mcp.tool
async def search_ifit_library(ctx: Context, query: str, workout_type: str = "", limit: int = 10) -> dict:
    ...
    return await _wrap(health_tools.search_ifit_library, query, workout_type, limit)


@mcp.tool
async def get_ifit_workout_details(ctx: Context, workout_id: str) -> dict:
    ...
    return await _wrap(health_tools.get_ifit_workout_details, workout_id)


@mcp.tool
async def search_ifit_programs(ctx: Context, query: str, limit: int = 10) -> dict:
    ...
    return await _wrap(health_tools.search_ifit_programs, query, limit)


@mcp.tool
async def get_ifit_program_details(ctx: Context, series_id: str) -> dict:
    ...
    return await _wrap(health_tools.get_ifit_program_details, series_id)


@mcp.tool
async def discover_ifit_series(ctx: Context, workout_id: str) -> dict:
    ...
    return await _wrap(health_tools.discover_ifit_series, workout_id)


@mcp.tool
async def report_exercise_correction(ctx: Context, workout_id: str, feedback: str) -> dict:
    ...
    return await _wrap(health_tools.report_exercise_correction, _uslug(ctx), workout_id, feedback)


@mcp.tool
async def recommend_strength_workout(ctx: Context) -> dict:
    ...
    return await _wrap(health_tools.recommend_strength_workout, _uslug(ctx))


@mcp.tool
async def create_hevy_routine_from_recommendation(
...
@mcp.tool
async def get_hevy_routine_review(
...
@mcp.tool
async def compare_hevy_workout(
...
@mcp.tool
async def apply_exercise_feedback(
...
@mcp.tool
async def get_routine_weight_recommendations(
